...
# Settings
py_files = ['main.py','client.py','server.py']
...
def run():
    ...
    try:
        ...
        if len(py_files) > 0:
            ...
            # execute the code
            ssh.exec_command(f'cd {remote_current_working_directory};rm -rf file_dict* log/* share/vma* share/vmb*')

            stdin, stdout, stderr = ssh.exec_command(f'cd {remote_current_working_directory}; {remote_python_interpreter} {main_py} --ip=192.168.56.101,192.168.56.103', bufsize=1, get_pty=True)
